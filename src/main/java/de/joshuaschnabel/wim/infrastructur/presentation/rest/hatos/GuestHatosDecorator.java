...
public class GuestHatosDecorator {
	...
	private final GuestController controller = WebFluxLinkBuilder.methodOn(GuestController.class);
	...
	private Mono<EntityModel<GuestDTO>> addLinksToDTO(GuestDTO dto) {
		final var selfLink = linkTo(this.controller.findGuest(dto.getId())).withSelfRel() //
				.andAffordance(this.controller.updateGuest(null, dto.getId()))//
				.andAffordance(this.controller.deleteGuest(dto.getId()))//
				.toMono();
		final var employeesLink = linkTo(this.controller.all()).withRel("guests") //
				.toMono();
		final var deleteLink = linkTo(this.controller.deleteGuest(dto.getId())).withRel("delete") //
				.toMono();
		final List<Mono<Link>> links = List.of(selfLink, employeesLink, deleteLink);
		return Flux.concat(links).collectList().map(link -> EntityModel.of(dto, link));
	}

	public Mono<CollectionModel<EntityModel<GuestDTO>>> addListLinks(Flux<GuestDTO> elements) {
		final Flux<EntityModel<GuestDTO>> flatMap = elements.flatMap(this::addLinksToDTO);
		return flatMap //
				.collectList() //
				.flatMap(resources -> linkTo(this.controller.all()).withSelfRel() //
						.andAffordance(this.controller.createGuest(null)).toMono() //
