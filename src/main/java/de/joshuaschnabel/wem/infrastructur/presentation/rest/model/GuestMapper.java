...
public class GuestMapper {
  ...
  private static final class MapperDTOToGuest implements Function<GuestDTO, Guest> {
    @Override
    public Guest apply(GuestDTO dto) {
      final var id = mapDtoIdToGuest.apply(dto.getId());
      return Guest.builder().id(id).name(this.mapToName(dto)).type(this.mapTypeToDTO(dto)).build();
    }
    ...
    private GuestType mapTypeToDTO(GuestDTO guest) {
      return GuestType.valueOf(guest.getType());
    }
  }

  private static final class MapperGuestToDTO implements Function<Guest, GuestDTO> {
    @Override
    public GuestDTO apply(Guest guest) {
      return GuestDTO.of(this.mapIdToDTO(guest), this.mapFirstnameToDTO(guest),
          this.mapLastnameToDTO(guest), this.mapTypeToDTO(guest));
    }
