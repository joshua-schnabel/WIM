...
class GuestDtoToDomainTest {
	...
	void fullDTO() {
		final var guestDto = GuestDTO.of("abcdefgh2345abcd", "Hans", "Wurst",
				"PrimaryGuest");
	...
	private Condition<? super Guest> hasId(String id) {
		return new Condition<Guest>(m -> m.getId().get().equals(id),
				"id " + id);
	}
	...
	private Condition<? super Guest> hasName(String first, String last) {
		return new Condition<Guest>(
				m -> m.getName()
						.map(name -> name.getFirstname().equals(first)
	...
	private Condition<? super Guest> hasType(String type) {
		return new Condition<Guest>(m -> m.getType().name().equals(type),
				"type " + type);
	}
	...
	void missingNameDTO() {
		final var guestDto = GuestDTO.of("abcdefgh2345abcd", null, null,
				"PrimaryGuest");
		final var guest = GuestMapper.mapDtoToGuest.apply(guestDto);
		assertThat(guest).has(this.hasId("abcdefgh2345abcd"));
